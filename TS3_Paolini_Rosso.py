...
f1 = (N/4) * delta_f                   # k0 = N/4 → f0 exacta en un bin
f2 = (N/4 + 0.25) * delta_f            # desintonía 0.25 Δf
f3 = (N/4 + 0.5) * delta_f             # desintonía 0.5 Δf

# ----------- Señales en tiempo -----------
#Amplitud = sqrt(2) para que la varianza sea 1 (potencia unitaria)
...
plt.plot(ff_eje, 10*np.log10(PSD1), label="f1 = N/4 Δf")
plt.plot(ff_eje, 10*np.log10(PSD2), label="f2 = (N/4 + 0.25) Δf")
plt.plot(ff_eje, 10*np.log10(PSD3), label="f3 = (N/4 + 0.5) Δf")
plt.xlim([230, 270])   # zoom en la zona de interés
...
#señal 1
energia_tiempo_1 = np.sum(np.abs(x1)**2)
energia_frec_1   = np.sum(PSD1)
varianza_1       = np.var(x1)
print("Señal 1:")
print(f"  Varianza (potencia) = {varianza_1:.4f}")
print(f"  Energía tiempo = {energia_tiempo_1:.4f}")
print(f"  Energía frec.  = {energia_frec_1:.4f}")

if np.isclose(energia_tiempo_1, energia_frec_1):
    print("Se cumple Parseval.")
else:
    print("Parseval no se cumple.")
...
varianza_2       = np.var(x2)
print("Señal 2:")
print(f"  Varianza (potencia) = {varianza_2:.4f}")
print(f"  Energía tiempo = {energia_tiempo_2:.4f}")
print(f"  Energía frec.  = {energia_frec_2:.4f}")

if np.isclose(energia_tiempo_2, energia_frec_2):
    print("Se cumple Parseval.")
else:
    print("Parseval no se cumple.")
...
varianza_3       = np.var(x3)
print("Señal 3:")
print(f"  Varianza (potencia) = {varianza_3:.4f}")
print(f"  Energía tiempo = {energia_tiempo_3:.4f}")
print(f"  Energía frec.  = {energia_frec_3:.4f}")

if np.isclose(energia_tiempo_3, energia_frec_3):
    print("Se cumple Parseval.")
else:
    print("Parseval no se cumple.")
...
def plot_freq_response(b, a, fs, titulo='Sistema LTI'):
...
plot_freq_response(b, a, fs, 'Sistema original')
...
aA = [1]
plot_freq_response(bA, aA, fs, 'Sistema A')
...
plot_freq_response(bB, aB, fs, 'Sistema B')
